...
curr_configs_bad = [
    {
        "topology": "ladder",
        "n_qubits": 10,
        "pass_rate_dual_criterion": 0.60,
    },  # drop 0.35 → regression
...
check(
    "regression detected when topology max drops 0.35",
    len(result) == 1 and result[0]["topology"] == "ladder",
)
...
with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
    ...
    corrupt_path = f.name
result = detect_pass_rate_regression(curr_small_drop, previous_dashboard_path=corrupt_path)
...
configs_with_mask = [
    {
        ...
        "h_frontier": 3.1,
        "theta_smoothness": 0.07,
        "zoo_vs_npz_divergence": 0.2,
        "model_stale": False,
    }
]
