...
**Generated:** 2026-06-17T01:32:40+00:00
**Results dir:** `results`
...
| Noisy/ZNE runs | 93 |
| Experiments | 54 |
| **Total result files** | **476** |

## Experiment Verdicts

- ✅ Confirmed: **33** | ⚠️ Rejected: **8** | ❌ Failed: **13**
- Useful-outcome rate: **76%**
...
| B4_BR_CROSS_N | ❌ failed | 83% | ΔE/gap < 5% |
| BOND_RESOLVED_HVA | ✅ confirmed | 100% | Bond-resolved HVA achieves dE/gap <= global on >=75% topologies |
| BOND_RESOLVED_SCALING | ✅ confirmed | 100% | Bond-resolved scales to 2D square + MPNN + ZNE (>=3/4 sections pass) |
| C1 | ❌ failed | 100% | Physics loss < 5% |
| C3 | ❌ failed | 67% | N=20 VQE < 5% |
| CROSS_TOPO_NOISY | ❌ failed | 100% | ΔE/gap < 5% |
| D1 | ✅ confirmed | 1% | Gradient peak detected near h_c |
| E3 | ❌ failed | 100% | ΔE/gap < 5% |
| E3_BR_SCALING | ❌ failed | 50% | Bond-resolved N=40 VQE converges (>=7/9 h-points ΔE/gap < 5%) |
...
| E5_SCALING_EXT | ✅ confirmed | 100% | Scaling extensions: ≥3/5 sections pass (bond-dim + VQE + HE + NLCE) |
...
| HW_REHEARSAL_V2 | ✅ confirmed | 100% | HardwareBackend(fake_backend) with PEA/GF/adaptive ZNE |
| HW_REHEARSAL_V3 | ❌ failed | 84% | ΔE/gap < 5% |
| MPS_HW | ✅ confirmed | 100% | MPS chi-proxy matches hardware |
| N16_SQUARE_DMRG2D | ✅ confirmed | 50% | N=16 DMRG 2D works + VQE convergence assessment (1/2 = DMRG validated) |
| PEA_CROSS_DENSE | ❌ failed | 100% | ΔE/gap < 5% |
...
| PEA_SCALING | ❌ failed | 50% | ΔE/gap < 5% |
| PEA_TRIANGULAR | ✅ confirmed | 100% | PEA-ZNE gain>0 on triangular N=6 with R²>0.9 (all seeds) |
...
| T1a_dense | ✅ confirmed | 100% | Dense J₂ grid (8 values) enables cross-J₂ interpolation >50% |
...
- Total compute: **19.0 hours** across 466 runs
- Per-run: mean=146.8s, median=27.6s, max=11085.5s

## Actionable Items

1. 🟠 **[HIGH]** 6 defined experiment(s) have no results
   - Missing: E5_BOND_DIM, E5_HE_COMPARISON, E5_NLCE, EXT1B, SCALE_N40, SCALE_N50
2. 🟡 **[MEDIUM]** ZNE validation missing for 2 config(s)
   - p=1 noiseless exists but no ZNE: chain_1d N=6, ladder N=6
3. 🟡 **[MEDIUM]** VQE chain breaks: 96/329 runs (29%) have θ-smoothness > 1.0
...
4. 🟡 **[MEDIUM]** VQE convergence issue: worst case only 75%
   - Mean convergence: 99.58%. Runs with < 80% convergence may need more restarts or higher max_iterations.
5. 🟡 **[MEDIUM]** MPNN overfitting: 41/279 runs (15%) have gen_gap > 0.01
...
6. 🟡 **[MEDIUM]** p=1 under-represented: only 57/329 (17%) of noiseless runs
   - p=1 is the recommended hardware strategy. Thesis generalization claims need balanced p=1 coverage across topologies.
7. 🟡 **[MEDIUM]** 13 experiment(s) partially passed — may improve with tuning
   - Near-threshold: B1(12%), B4_BR_CROSS_N(83%), C1(100%), C3(67%), CROSS_TOPO_NOISY(100%), E3(100%), E3_BR_SCALING(50%), HW_REHEARSAL_V3(84%). These have partial success and may cross threshold with optimized configs (more restarts, adjusted h-grid, larger N).
8. ⚪ **[LOW]** 4 result(s) tested outside valid regime
   - These are expected to fail — not actionable unless they passed unexpectedly.
9. ⚪ **[LOW]** MPNN dominates error: 100% of total ΔE comes from MPNN prediction
   - Mean circuit error: 0.0000, Mean MPNN error: 1.2235. Circuit expressibility is not the bottleneck — focus improvement on MPNN architecture/training.
