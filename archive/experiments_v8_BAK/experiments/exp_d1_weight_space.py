...
location differs between MPNN trained on full h-range (detects h_c) vs
valid-regime-only (detects training boundary).

Method:
    1. Train MPNN-A on full h-range [0.5, 2.5] (includes invalid regime)
...
class ExperimentD1(BaseExperiment):
    ...
    def default_config(cls) -> ExperimentConfig:
        return ExperimentConfig(
            ...
            system=SystemConfig(
                n_qubits=6,
                p_layers=2,
                h_values=[],  # Will be set per variant
            ),
            mpnn=MPNNConfig(
                hidden_dim=64,
                n_layers=3,
    ...
    def setup(self) -> None:
        """Override: we need MPNN infrastructure, not just circuits."""
    ...
    def run_single(self, seed: int) -> list[V8Metrics]:
        ...
        # Generate VQE data across full h-range
        ...
        # Train MPNN-A (full range) and MPNN-B (valid only)
        mpnn_a, history_a = self._train_mpnn(h_full, theta_full, seed, tag="full")
        mpnn_b, history_b = self._train_mpnn(h_valid, theta_valid, seed, tag="valid")

        # Compute weight gradients ||dW/dh||
        ...
        # Build metrics (one per probe point for MPNN-A)
        metrics = []
        for i, h in enumerate(h_probe):
            m = V8Metrics(
                ...
                energy_error=0.0,
                gap=1.0,
                relative_error=float(grad_a[i]),  # Use as "error" for aggregation
                seed=seed,
                wall_time_s=0.0,
                technique_metadata={
                    "grad_norm_full_range": float(grad_a[i]),
                    "grad_norm_valid_only": float(grad_b[i]) if i < len(grad_b) else 0.0,
                    "peak_h_full": peak_a_h,
                    "peak_h_valid": peak_b_h,
                    "known_h_c": 1.0,
                    "distance_from_hc_full": abs(peak_a_h - 1.0),
                    "distance_from_hc_valid": abs(peak_b_h - 1.0),
                },
            )
            metrics.append(m)

        self.logger.info(
            f"  Seed {seed}: peak_A={peak_a_h:.2f} (full), peak_B={peak_b_h:.2f} (valid), h_c=1.0"
        )
    ...
    def _train_mpnn(self, h_values, theta_data, seed, tag=""):
        """Train a simple MLP as MPNN proxy for weight gradient analysis."""
        import torch
        import torch.nn as nn

        torch.manual_seed(seed)

        # Simple MLP (proxy for full MPNN — same weight structure)
        n_params = theta_data.shape[1]
        model = nn.Sequential(
            nn.Linear(1, 64),
            nn.ReLU(),
            nn.Linear(64, 64),
        ...
        history = []
        for epoch in range(self.config.mpnn.n_epochs):
            optimizer.zero_grad()
            pred = model(X)
            loss = loss_fn(pred, Y)
            loss.backward()
        ...
        self.logger.info(f"    MPNN-{tag}: final loss={history[-1]:.6f}")
        return model, history

    def _compute_weight_gradients(self, model, h_probe, N):
        """Compute ||dW/dh|| at each probe point via finite differences."""
        import torch
    ...
    def analyze(self, results: dict[int, list[V8Metrics]]) -> dict:
        ...
        # Collect peaks across seeds
        peaks_full = []
        peaks_valid = []

        for _seed, metrics in results.items():
            if metrics:
                md = metrics[0].technique_metadata
                peaks_full.append(md["peak_h_full"])
                peaks_valid.append(md["peak_h_valid"])

        analysis["phase_detection"] = {
            "mean_peak_full_range": float(np.mean(peaks_full)) if peaks_full else 0,
            "std_peak_full_range": float(np.std(peaks_full)) if peaks_full else 0,
            "mean_peak_valid_only": float(np.mean(peaks_valid)) if peaks_valid else 0,
            "std_peak_valid_only": float(np.std(peaks_valid)) if peaks_valid else 0,
            "known_h_c": 1.0,
            "full_range_detects_hc": (
                abs(float(np.mean(peaks_full)) - 1.0) < 0.3 if peaks_full else False
            ),
            "valid_only_detects_boundary": (
                float(np.mean(peaks_valid)) > 1.2 if peaks_valid else False
            ),
        }
    ...
    def report(self, analysis: dict) -> str:
        ...
        lines = [base, "", "Phase Detection from Weight Space:"]

        pd = analysis.get("phase_detection", {})
        lines.extend(
            [
                f"  MPNN-A (full range):  peak at h = {pd.get('mean_peak_full_range', 0):.2f} "
                f"± {pd.get('std_peak_full_range', 0):.2f}",
                f"  MPNN-B (valid only):  peak at h = {pd.get('mean_peak_valid_only', 0):.2f} "
                f"± {pd.get('std_peak_valid_only', 0):.2f}",
                "  Known h_c:            1.00",
                "",
                f"  Full-range detects h_c (within ±0.3): {pd.get('full_range_detects_hc', False)}",
                f"  Valid-only detects boundary (>1.2):   {pd.get('valid_only_detects_boundary', False)}",
            ]
        )
