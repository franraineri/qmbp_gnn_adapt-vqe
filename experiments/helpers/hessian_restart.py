...
from qmbp_simulation.analysis import compute_hessian


def _analyze_critical_point(
    cost_fn: Callable[[np.ndarray], float],
    point: np.ndarray,
    epsilon: float = 1e-4,
) -> dict:
    """Analyze a critical point via Hessian eigendecomposition.
    ...
    point : np.ndarray
        Parameter vector at the critical point.
    epsilon : float
        Finite difference step for Hessian computation.

    Returns
    -------
    dict with keys: eigenvalues, eigenvectors, type, n_negative,
                    condition_number, escape_direction
    """
    hessian = compute_hessian(cost_fn, point, epsilon)
    eigenvalues, eigenvectors = np.linalg.eigh(hessian)

    n_negative = int(np.sum(eigenvalues < -1e-6))
    ...
    # Escape direction: along most negative eigenvector
    escape_direction = None
    if n_negative > 0:
        min_idx = int(np.argmin(eigenvalues))
        escape_direction = eigenvectors[:, min_idx]

    # Condition number (ratio of max to min positive eigenvalue)
...
def hessian_guided_vqe(
    ...
    for restart in range(max_restarts):
        ...
        analysis = _analyze_critical_point(cost_fn, result.x, hessian_epsilon)
