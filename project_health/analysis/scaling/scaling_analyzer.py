...
def format_report_text(report: ScalingReport) -> str:
    ...
    if report.mode_comparison:
        ...
        lines.append(f"  Modes: {'✅ CONSISTENT' if consistent else '⚠️  DIVERGENT'}")
        lines.append(f"  Mean speedup: {summary.get('mean_speedup', 0):.1f}×")
        lines.append(f"  Mean energy diff: {summary.get('mean_energy_diff', 0):.6f}")
        lines.append(
    ...
    lines.append("  Key: MPS χ=64 exact for HVA p≤2 on 1D TFIM at all N tested")
    lines.append("")
