...
# From preflight cost model (Heron R2 effective CLOPS at N=10, depth~25)
# Note: actual QPU time is base × n_noise_factors × IBM_overhead (2.5×)
# For PEA configs, add noise learning time.
COST_MODEL = {
    "effective_clops": 3750,
    "shots": 16384,
    "time_per_circuit_s": 16384 / 3750,  # ~4.37s (single circuit, no ZNE)
    "ibm_overhead_factor": 2.5,  # Compilation + scheduling + classical
}
...
def check_qpu_time(name, d):
    ...
    # Check wall/QPU ratio (expected 1.5-10×, anything beyond suggests issues)
    if wall and qpu and qpu > 0:
        ratio = wall / qpu
        if ratio > 10:
            issues.append(f"wall/qpu ratio={ratio:.1f}× (>10× suggests congested queue)")

    # Validate classical_overhead_s if present
    classical = timing.get("classical_overhead_s")
    if classical is not None and classical < 0:
        issues.append(f"classical_overhead_s={classical:.1f}s is negative (data error)")
...
def main():
    ...
    for name, d in results:
        ...
        if circ_issues:
            ...
            cs = d.get("circuit_stats", {})
            print(
                f"  ✓ Circuit: n_2q={cs.get('n_2q_gates')}, "
        ...
        if qpu:
            ...
            print(
                f"    Model predicted: {predicted_time:.1f}s, actual: {qpu}s (ratio={ratio:.1f}x)"
            )
        ...
        if energy_issues:
            ...
            de_gap = res.get("delta_e_gap")
            print(
                f"  ✓ Energy: e_raw={e_raw:.4f}, e_exact={e_exact:.4f}, ΔE/gap={de_gap * 100:.2f}%"
            )
    ...
    for s in sorted(summary, key=lambda x: (x["h"], x["config"])):
        ...
        print(f"  {s['config']:<25s} {s['h']:>4} {de:>8s} {e:>10s} {qpu_str:>7s} {wall_str:>8s}")
