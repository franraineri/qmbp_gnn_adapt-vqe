...
```json
{
  ...
  "metadata": { "python_version", "qiskit_version", "torch_version", "git_commit", ... },
  "simulation_diagnostics": {
    "backend_type": "noiseless_statevector | mps_aer_mps_chi64_exact | hardware_ibm_kingston | fake_torino",
    "n_qubits": 10,
    "topology": "heavy_hex",
    "method_exact": true,
    "chi_max": 64,
    "chi_sufficiency_warning": "...(only if 2D + N>16)...",
    "shots": 16384,
    "hardware_mode": "fake_backend | hardware",
...
**New fields (2026-07-13):**
- `simulation_diagnostics`: Auto-injected by `_build_envelope()`. Documents backend used.
- `metadata.git_commit`: Short SHA of the commit that generated this result.
- VQE per-point: `variational_violation` (float), `variational_ok` (bool).
- VQE topology summary: `n_variational_violations`, `max_variational_violation`.
- Auto-abort: VQE sweep aborts if >4 consecutive variational violations detected.
...
| `python project_health/cli/query_index.py --backend mps` | Filter runs by backend type (mps/noiseless/hardware) |
| `python scripts/experiment_runners/scaling/run_scaling_validation.py --verify-chi` | Chi-convergence test (MANDATORY for 2D N>16) |
...
# MPS Scaling validation (N=40-200, chain_1d)
.venv/bin/python scripts/experiment_runners/scaling/run_scaling_validation.py \
    --n-qubits 40 --h-points 8

# MPS Scaling with chi-convergence verification (thesis-grade)
.venv/bin/python scripts/experiment_runners/scaling/run_scaling_validation.py \
    --n-qubits 40 --h-points 8 --verify-chi

# MPNN training on scaling data
.venv/bin/python scripts/experiment_runners/scaling/run_scaling_phase3_mpnn.py \
    --result-file results/scaling/scaling_N40_*.json

# Quick health check after running
python -m project_health --diagnose --model tfim
```

### Post-Run Analysis (ALWAYS do after thesis-grade runs)

```bash
# 1. Diagnose results
python -m project_health --diagnose --model tfim

# 2. Verify thesis integrity
python scripts/verify_thesis_runs.py

# 3. Check for fragile passes (energy_variance > 0.5 on passing points)
# → Automatically reported in section_2.data.topologies.*.n_fragile_passes

# 4. Verify simulation method recorded
# → Check simulation_diagnostics.backend_type in the result JSON
```
