...
def _validate_dataset_integrity(data: dict, filepath: str | Path) -> None:
    ...
    if theta_opt is not None:
        ...
        if theta_opt.ndim == 2 and theta_opt.shape[0] != n_points:
            raise ValueError(f"Dataset at '{filepath}': theta_opt has {theta_opt.shape[0]} rows, expected {n_points}.")
    ...
    for name in ("ground_energies", "vqe_energies", "gaps"):
        ...
        if n_bad > 0:
            raise ValueError(
                f"Dataset at '{filepath}': array '{name}' contains {n_bad} NaN/Inf values. Dataset is corrupted."
            )
...
# ═══════════════════════════════════════════════════════════════════════════════
# h-grid generation — moved to qmbp_simulation.utils.h_grid (pure numpy util).
# Re-exported here for backward compatibility; the single source of truth is
# utils.h_grid, so analysis/framework can build grids without importing pipeline.
# ═══════════════════════════════════════════════════════════════════════════════

from qmbp_simulation.utils.h_grid import (  # noqa: E402,F401
    generate_frontier_dense_h_grid,
    generate_nonuniform_h_grid,
)
