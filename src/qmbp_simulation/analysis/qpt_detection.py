...
This is the domain-logic home for QPT detection. The CLI wrapper lives at
``scripts/analysis/qpt_detection.py`` and re-exports this module's public API.

Public API
----------
- ``get_h_critical(topology, n_qubits, p_layers)`` — main integration point
  for other modules that need h_c (quality_profile, retrain triggers, runners).
- ``run_qpt_analysis(...)`` — full detection pipeline for a topology.
- ``load_energy_curves(...)`` — E(h) curves for all available N.
- ``compute_second_derivative(...)``, ``find_critical_field(...)``,
  ``finite_size_scaling(...)`` — lower-level building blocks.
"""
...
def load_energy_curves(
    ...
    *,
    ...
    # Each DQPT run stores e_exact_h_pre (ground state energy at the pre-quench
    # field). Scanning runs with different h_pre yields free E(h) points that
    # supplement sparse QPT coverage (especially N=12-18 heavy_hex).
...
def get_h_critical(
    ...
    Main integration point for modules that need h_c (e.g.,
    compute_quality_profile for regional breakdown, retrain triggers, runners).
...
def find_critical_field(
    ...
    *,
    ...
    -------
    tuple[float, float]
        (h_c, peak_magnitude) — critical field and |d2E| at that point.
    """
    ...
    # corresponding to the inflection point of E(h) — maximum curvature.
...
def finite_size_scaling(h_c_by_n: dict[int, float]) -> dict:
    ...
    # The 3-parameter law h_inf + a/N^nu needs at least 3 distinct N-values with
    # variation in h_c to be identifiable. If h_c is flat (h_c converges fast —
    # good physics), curve_fit cannot estimate covariance and emits
    # OptimizeWarning. We detect this and downgrade to a robust median estimate.
    import warnings
...
def run_qpt_analysis(
    ...
    *,
    ...
    for n in sorted(curves.keys()):
        ...
        # (insufficient coverage) or without >=0.3 h-range on each side of h_c
