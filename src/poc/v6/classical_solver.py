...
class ClassicalSolver:
    ...
    # ── Task 3.1: solve() with auto method selection ──────────────────
    ...
    # ── Task 3.2: exact diagonalization ───────────────────────────────
    ...
    def _solve_dmrg(
        ...
        # TeNPy 1.x's orthogonal_to is unreliable.  For finite chains,
        # use the excitation gap from the DMRG engine's energy history
        # or a second run with a different initial state.
        gap = 0.0
        try:
            # Approach: run a second DMRG with orthogonal initial state.
            # If orthogonal_to is not supported, fall back to gap=0.
            ...
            e1, _ = eng_ex.run()
            # The second run without orthogonal projection may find the same
        ...
        # or fall back to XX correlation: ⟨X⟩_eff = sqrt(|⟨X_i X_j⟩|) for
        # bulk sites.  For the PoC this is acceptable; exact diag is used
        # for N<15 where this issue doesn't arise.
