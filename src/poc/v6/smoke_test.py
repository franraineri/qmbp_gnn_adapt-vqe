...
all modules work correctly.  Expected runtime: ~2 minutes.
...
# Ensure project root is on sys.path so `src.poc.v6` imports work
...
def main() -> int:
    ...
    from src.poc.v6.pipeline_utils import (
        save_phase12_dataset, load_phase12_dataset, assert_observable_locality,
    )
    ...
    model = MPNNPredictor(
        node_features=2, hidden_dim=32, n_layers=2, output_dim=2 * p_layers,
    )
