...
class TestHeisenbergPipelineIntegration:
    ...
    def test_phase1_heisenberg_produces_different_energies_than_tfim(
        ...
        runner_heis = PipelineRunner(chain_lattice, config, model_spec=heisenberg_spec, seed=SEED)
        runner_tfim = PipelineRunner(chain_lattice, config, model_spec=tfim_spec, seed=SEED)
        ...
        for r_h, r_t in zip(results_heis, results_tfim, strict=False):
            assert r_h.ground_energy != pytest.approx(r_t.ground_energy, abs=1e-6)

    def test_phase2_heisenberg_produces_8_params(self, chain_lattice, heisenberg_spec):
        """VQE results have 8 parameters (4 per layer × 2 layers)."""
        h_values = np.array([2.0, 1.5])
        config = VQEConfig(p_layers=2, n_restarts=1, maxiter=50)

        runner = PipelineRunner(chain_lattice, config, model_spec=heisenberg_spec, seed=SEED)
    ...
    def test_pipeline_model_spec_dispatch_circuit(self, chain_lattice, heisenberg_spec, tfim_spec):
    ...
    def test_n_variational_params_helper(self, chain_lattice, heisenberg_spec, tfim_spec):
        ...
        runner_h1 = PipelineRunner(chain_lattice, config_p1, model_spec=heisenberg_spec, seed=SEED)
        runner_h2 = PipelineRunner(chain_lattice, config_p2, model_spec=heisenberg_spec, seed=SEED)
        assert runner_h1._n_variational_params() == 4
        assert runner_h2._n_variational_params() == 8

        # TFIM p=1 → 2, p=2 → 4
        runner_t1 = PipelineRunner(chain_lattice, config_p1, model_spec=tfim_spec, seed=SEED)
        runner_t2 = PipelineRunner(chain_lattice, config_p2, model_spec=tfim_spec, seed=SEED)
    ...
    def test_phase3_output_dim_matches_model(self, chain_lattice, heisenberg_spec):
