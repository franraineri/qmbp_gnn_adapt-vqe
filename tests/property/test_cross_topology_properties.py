...
# Strategies for orchestrator cache skip tests
p_layers_st = st.sampled_from([1, 2])
file_suffix_st = st.text(
...
class TestOrchestratorCacheSkip:
    """Property 11: Orchestrator Cache Skip.

    **Validates: Requirements 7.3**

    For any expected data file path that already exists on disk, the
    orchestrator SHALL NOT invoke the VQE generator for that topology/size
    combination when `skip_existing=True`.
    """
    ...
    @given(
        ...
        n=system_size_st,
        p=p_layers_st,
        suffix=file_suffix_st,
    )
    @settings(max_examples=50, deadline=None)
    def test_existing_file_detected_by_check_data_exists(
        self,
        topology: str,
        n: int,
        p: int,
        ...
        """check_data_exists returns True when a matching file exists."""
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            check_data_exists,
        )

        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            filename = f"vqe_{topology}_N{n}_p{p}_{suffix}.json"
            (tmp_path / filename).write_text("{}")
            assert check_data_exists(tmp_path, topology, n, p) is True
    ...
    @settings(max_examples=50, deadline=None)
    def test_missing_file_not_detected(
        self,
        topology: str,
        n: int,
        p: int,
    ):
        """check_data_exists returns False when no matching file exists."""
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            check_data_exists,
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            assert check_data_exists(tmp_path, topology, n, p) is False
    ...
    @settings(max_examples=50, deadline=None)
    def test_skip_existing_true_does_not_call_generator_when_file_exists(
        self,
        topology: str,
        n: int,
        p: int,
        ...
        """When skip_existing=True and data file exists, VQE generator is NOT called.

        We verify the skip logic: skip_existing=True + file exists -> skip.
        """
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            check_data_exists,
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            exists = check_data_exists(output_dir, topology, n, p)
            assert exists is True

            skip_existing = True
            should_skip = skip_existing and check_data_exists(output_dir, topology, n, p)
    ...
    @settings(max_examples=50, deadline=None)
    def test_skip_existing_true_calls_generator_when_file_missing(
        self,
        topology: str,
        n: int,
        p: int,
    ):
        """When skip_existing=True but data file is missing, VQE generator IS called.

        The skip condition only activates when both skip_existing=True AND the
        file actually exists. If the file is missing, generation must proceed.
        """
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            check_data_exists,
        )
    ...
    @settings(max_examples=50, deadline=None)
    def test_skip_existing_false_always_calls_generator(
        self,
        topology: str,
        n: int,
        p: int,
        ...
        """When skip_existing=False, VQE generator is ALWAYS called regardless of files.

        Even if the data file exists, setting skip_existing=False means the
        orchestrator must regenerate data.
        """
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            check_data_exists,
        )
    ...
    @settings(max_examples=30, deadline=None)
    def test_orchestrator_integration_skip_existing_with_mock(
        self,
        suffix: str,
    ):
        """Integration: run_orchestrator with skip_existing=True skips generation
        when files already exist in output_dir (mocked subprocess execution).

        Patches run_step to track which steps are actually invoked.
        """
        from scripts.experiment_runners.cross_topology.run_orchestrator import (
            run_orchestrator,
        )
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Create matching data files for heavy_hex and triangular at N=6,16
            for topo in ["heavy_hex", "triangular"]:
                for n_size in [6, 16]:
                    fname = f"vqe_{topo}_N{n_size}_p1_{suffix}.json"
                    (output_dir / fname).write_text("{}")

            # Mock run_step so no actual subprocess is executed
            with patch(
                "scripts.experiment_runners.cross_topology.run_orchestrator.run_step"
            ) as mock_run_step:
            ...
            # Verify that NO data_gen step was called via run_step
            # (because all files exist and skip_existing=True)
            for call in mock_run_step.call_args_list:
                step_name = call[1].get("step_name", call[0][0] if call[0] else "")
                assert not step_name.startswith("data_gen_"), (
                    f"VQE generator was invoked for {step_name} despite "
                    f"skip_existing=True and all data files existing"
                )
