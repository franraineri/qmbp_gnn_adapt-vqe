...
class TestMPSExactMode:
    ...
    def test_exact_matches_statevector(self, tfim_n6):
        ...
        mps_backend = MPSBackend(
            strategy="aer_mps", chi_max=64, deterministic=True, seed=42
        )
    ...
    def test_cache_handles_different_qubit_counts(self, tfim_n6, tfim_n10):
        ...
        assert e6 == e6_again, (
            f"Cache invalidation broke reproducibility: {e6} != {e6_again}"
        )
...
class TestMPSStochasticMode:
    ...
    def test_stochastic_mode_runs(self, tfim_n6):
        ...
        backend = MPSBackend(
            strategy="aer_mps", chi_max=64, precision=0.005,
    ...
    def test_stochastic_has_noise(self, tfim_n6):
        ...
        backend = MPSBackend(
            strategy="aer_mps", chi_max=64, precision=0.01,
            deterministic=False, seed=None,  # No fixed seed → different each time
        )
...
class TestMPSPerformance:
    ...
    def test_exact_mode_fast(self, tfim_n6):
        ...
        assert elapsed < 5.0, (
            f"20 evals took {elapsed:.1f}s ({ms_per_eval:.0f}ms/eval) — too slow"
        )
...
class TestMPSDefaultBehavior:
    ...
    def test_default_is_deterministic(self, tfim_n6):
        ...
        assert len(set(results)) == 1, (
            f"Default mode is not deterministic: {results}"
        )
