...
class TestGenerateModelQualityDashboard:
    ...
    def test_dashboard_overwrites_file(self, tmp_path):
        ...
        # Set mtime to very old so freshness check triggers regeneration
        os.utime(out, (0, 0))
...
class TestComputeHFrontierFromNPZ:
    ...
    def test_valid_npz_with_all_fields(self, tmp_path):
        ...
        h = np.array([2.0, 2.5, 3.0, 3.5, 4.0])
        e_exact = np.array([-10.0, -9.0, -8.0, -7.0, -6.0])
    ...
    def test_npz_without_de_gaps_computes_them(self, tmp_path):
        ...
        e_exact = np.array([-8.0, -7.0, -6.0])
        e_vqe = e_exact + np.array([0.1, 0.04, 0.01])
        gaps = np.array([2.0, 2.5, 3.0])

        path = tmp_path / "no_degaps.npz"
        np.savez(
            path, h_values=h, e_vqe=e_vqe, e_exact=e_exact, gaps=gaps, theta_opt=np.zeros((3, 11))
        )
    ...
    def test_npz_missing_energy_fields(self, tmp_path):
        ...
        np.savez(path, h_values=np.array([1.0, 2.0, 3.0]), theta_opt=np.zeros((3, 5)))
    ...
    def test_single_point_npz(self, tmp_path):
        ...
        np.savez(
            path,
            h_values=np.array([3.0]),
            e_vqe=np.array([-7.0]),
            e_exact=np.array([-7.1]),
            gaps=np.array([2.0]),
            de_gaps=np.array([0.05]),
...
class TestOverfittingDetection:
    ...
    def test_overfitting_stop_reason_in_result(self):
        ...
        def _make_graph(seed, scale=1.0):
            ...
            n_nodes = n_q + n_e + n_q
            x = torch.tensor(
                rng.standard_normal((n_nodes, UNIFIED_NODE_FEATURES)) * scale, dtype=torch.float32
            )
            node_type = torch.cat(
                [
                    torch.zeros(n_q, dtype=torch.long),
            ...
            edge_list = torch.tensor(
                [[i % n_q, (i + 1) % n_q] for i in range(n_e)], dtype=torch.long
            )
            g = Data(
                x=x,
        ...
        result = train_unified_mpnn(model, dataset, n_epochs=500, val_fraction=0.4, seed=42)
    ...
    def test_fine_tune_categorizes_overfitting(self):
        ...
        from qmbp_simulation.predictors.unified_graph import UNIFIED_NODE_FEATURES
        from qmbp_simulation.predictors.unified_mpnn import UnifiedMPNN, fine_tune_unified_mpnn
