...
\begin{enumerate}
    ...
    \item \textbf{OE5:} Validar sistemáticamente el pipeline sobre 5 topologías (cadena 1D, escalera, triangular, kagomé, heavy-hex) y tamaños $N = 6$ a $N = 80$, documentando reproducibilidad y límites físicos.
...
En total, se acumulan 210+ ejecuciones de pipeline documentadas en bitácoras estructuradas, con 30 experimentos formales y una tasa de resultados útiles del 93\% (véase Tabla~\ref{tab:experiment_summary} en Capítulo~5).
...
El framework logra $\Delta E/\text{gap} < 5\%$ en las cuatro topologías con los mismos hiperparámetros (Tabla~\ref{tab:cross_topo}). El ranking de rendimiento (escalera $<$ cadena $<$ triangular) se explica por la cantidad de información disponible para el paso de mensajes: topologías con mayor conectividad (más aristas por nodo) proporcionan más canales de propagación de información durante las capas GINConv, lo cual es consistente con la teoría de expresividad del test de Weisfeiler-Lehman \citep{xu2019}. La topología heavy-hex ---nativa de los procesadores IBM Torino \citep{kim2023}--- logra el mejor rendimiento absoluto (mediana 0.10\%) con la ventaja adicional de cero overhead de routing SWAP en hardware.
...
\section{Validación de Escalamiento ($N = 6$ a $N = 80$)}
...
\begin{table}[h]
\centering
\caption{Escalamiento del pipeline GNN-HVA ($p = 1$, cadena 1D). Para $N \leq 22$: \texttt{StatevectorEstimator}; para $N > 22$: simulación MPS ($\chi = 64$, COBYLA).}
\label{tab:scaling}
\begin{tabular}{rcccc}
\toprule
...
\noindent validada con error consistente de $\pm 0.50$ en $N = 40$, $50$ y $80$ (Tabla~\ref{tab:scaling}).
...
\begin{table}[h]
\centering
\caption{Validación de PEA-ZNE cross-topology ($p = 1$, $N = 10$, 3 semillas $\times$ 6 $h$-puntos por topología). Ref: \citet{kim2023}.}
\label{tab:pea_zne}
\begin{tabular}{lcccc}
\toprule
\textbf{Topología} & \textbf{Ganancia media} & \textbf{$R^2$ medio} & \textbf{Victorias} & \textbf{$t$-stat (vs. GF)} \\
\midrule
Cadena 1D & +97\% & 0.998 & 6/6 & $t = 46.3$, $p < 10^{-19}$ \\
Escalera & +91\% & 0.997 & 6/6 & — \\
Heavy-Hex & +98\% & 0.998 & 6/6 & — \\
Triangular & +97\% & 0.998 & 6/6 & — \\
\midrule
\textbf{Total} & \textbf{+95\%} & \textbf{0.998} & \textbf{18/18} & $p < 10^{-19}$ \\
\bottomrule
\end{tabular}
\end{table}

PEA-ZNE supera a gate-folding ZNE por un factor $4.6\times$ en ganancia media (95\% vs. 20.6\%) con $R^2 = 0.998$, validando su adopción como estrategia primaria de mitigación. El overhead en QPU es $\sim$50\% (fase de aprendizaje de ruido), justificado por la superioridad demostrada.
...
\subsection{Entrenamiento Noise-Aware de la MPNN Falla}

El resultado del experimento V7 5B (Sección anterior) tiene una implicación adicional: confirma que la generación de datos de entrenamiento de alta calidad es un prerequisito no negociable para el éxito del pipeline. Los datos VQE deben ser producidos en condiciones noiseless (o equivalentemente con MPS exacto) para que la MPNN aprenda la relación suave $h \mapsto \theta^*$ postulada por \citet{mele2022}. Cualquier perturbación estocástica en los targets destruye la señal aprendible.
...
El 69\% de los fallos es prevenible mediante el early-stopping implementado (Tabla~\ref{tab:root_cause}: CHAIN\_BREAK 45\% + MPNN\_OVERFIT 25\% detectable y prevenible). La tasa de resultados útiles (confirmados + negativos válidos) es del 93\% (28/30 experimentos formales producen conocimiento accionable), comparado con una tasa típica del 50--70\% reportada en estudios de benchmarking VQE \citep{tilly2022}.
...
\midrule
\textbf{GNN-HVA (este trabajo)} & \textbf{GINConv MPNN} & \textbf{TFIM (+ext)} & \textbf{80} & \textbf{50--500$\times$} & \textbf{Sí (5 topo)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Escala de validación:} El pipeline se valida hasta $N = 80$, mientras que los trabajos de predicción de parámetros revisados operan a $N \leq 12$ \citep{miao2024, zhang2025, zou2026}. La extensión a $N > 22$ requiere la transición a backends MPS, lo cual constituye una contribución de ingeniería.
    ...
    \item \textbf{Integración con mitigación de errores:} Se integra PEA-ZNE ($R^2 = 0.998$) como componente del pipeline, permitiendo evaluar el rendimiento end-to-end bajo ruido realista. La técnica PEA-ZNE fue propuesta por \citet{kim2023}; nuestra contribución es su integración y validación cruzada sobre 4 topologías.
...
Bond-resolved ($p=2$, 79 params) & Necesaria & No aplicable & MPNN única opción \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{3. La restricción de profundidad es \emph{necesaria} y \emph{suficiente}.} El resultado de \citet{mele2026} impone $p \leq O(\log n)$ como requisito de supervivencia al ruido. Para $N = 10$--$80$ en cadena 1D, $p = 1$--$2$ capas son $O(1) \subset O(\log n)$, satisfaciendo holgadamente la cota. Simultáneamente, el HVA $p \leq 2$ es suficiente para resolver la física fuera de la región crítica ($h \geq h_{\min}$), como confirman nuestros resultados de escalamiento (Tabla~\ref{tab:scaling}: $\Delta E/\text{gap} < 1\%$ a $N = 40$--$80$).
...
\begin{table}[h]
\centering
\caption{Resumen de la campaña experimental formal V8 (15 experimentos con hipótesis formales). Los 210+ ejecuciones totales incluyen además los experimentos de escalamiento (V7), Heisenberg (V9), S-series (S1-S8), y Tier-1 (T1a-T1c).}
...
Optimización (B) & 2 & 0 & 1 \\
Predictor (C, D, G) & 3 & 3 & 1 \\
Paisaje energético (F) & 1 & 1 & 0 \\
Generalización (E) & 0 & 1 & 0 \\
\midrule
\textbf{Total V8} & \textbf{8 (53\%)} & \textbf{5 (33\%)} & \textbf{2 (13\%)} \\
\midrule
\multicolumn{4}{l}{\textit{Campaña completa (V7+V8+V9+S+T1):}} \\
Total global & 22 & 8 & 2 \\
\bottomrule
\end{tabular}
\end{table}

La tasa de resultados útiles (confirmados + rechazados con resultado negativo válido) es del 87\%, demostrando la madurez de la metodología experimental. Los 5 experimentos rechazados producen conocimiento accionable: delimitan la aplicabilidad del framework (cross-topology transfer falla, Heisenberg falla, escalamiento de exponente crítico falla).
...
\begin{enumerate}
    ...
    \item \textbf{Escalabilidad validada:} El pipeline opera desde $N = 6$ hasta $N = 80$ sin modificaciones de código (Tabla~\ref{tab:scaling}). La técnica de simulación MPS para VQE fue propuesta anteriormente \citep{hauschild2018}; nuestra contribución es su integración transparente en el pipeline de 4 fases.

    \item \textbf{Mitigación de errores integrada:} PEA-ZNE \citep{kim2023} logra +95\% de ganancia con $R^2 = 0.998$ (Tabla~\ref{tab:pea_zne}). La técnica PEA es de \citet{kim2023}; validamos aquí su aplicabilidad cruzada sobre 4 topologías para nuestro tipo de circuito (HVA $p=1$).
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{El pipeline GNN-HVA clasifica correctamente fases cuánticas} con $\Delta E/\text{gap} < 5\%$ en el régimen operativo válido, validado sobre 210+ ejecuciones, 5 topologías y tamaños de sistema de $N = 6$ a $N = 80$ (Tablas~\ref{tab:cross_topo}--\ref{tab:scaling}). La tasa de aprobación es 100\% dentro del régimen válido ($h \geq h_{\min}$) para configuraciones optimizadas.
    ...
    \item \textbf{El framework escala sin modificaciones arquitectónicas} desde $N = 6$ (régimen de validación exacta) hasta $N = 80$ (más allá de la barrera del simulador de vector de estado), con precisión que \emph{mejora} con el tamaño del sistema ($\Delta E/\text{gap} = 0.08\%$ a $N = 80$, Tabla~\ref{tab:scaling}).

    \item \textbf{PEA-ZNE es la estrategia de mitigación óptima} para este pipeline: +95\% de ganancia media, $R^2 = 0.998$, validada en 4 topologías con superioridad estadística sobre gate-folding ZNE ($t = 46.3$, $p < 10^{-19}$, Tabla~\ref{tab:pea_zne}), consistente con la demostración original de \citet{kim2023}.
    ...
    \item \textbf{La generalización zero-shot cross-N funciona:} entrenando con datos de $N = 40 + 80$, el modelo predice correctamente para $N = 50$--$100$ (25/25 PASS, media $\Delta E/\text{gap} = 0.16\%$, Tabla~\ref{tab:cross_n}) con la corrección de \texttt{norm\_type="none"} para topologías con simetría nodal.
...
\begin{itemize}
    ...
    \item \textbf{OE5} (Validación sistemática): Completado --- 210+ ejecuciones, 5 topologías, $N = 6$--$80$, 30 experimentos formales.
    \item \textbf{OE6} (Hardware + mitigación): Completado en simulación ruidosa (PEA-ZNE +95\%, $R^2 = 0.998$). Pendiente: ejecución en QPU real (requiere credenciales IBM).
...
\noindent We present GNN-HVA, a hybrid classical-quantum pipeline for quantum phase classification on NISQ hardware. The framework combines a Message-Passing Neural Network (MPNN) based on GINConv with a depth-bounded Hamiltonian Variational Ansatz (HVA, $p \leq 2$) to classify quantum phases of the Transverse Field Ising Model with zero iterative quantum optimization. The MPNN predicts variational parameters directly from the Hamiltonian's graph representation, reducing QPU cost by $50$--$500\times$ compared to conventional VQE. Systematic validation over 210+ pipeline executions across 5 lattice topologies (chain, ladder, triangular, kagome, heavy-hex) and system sizes $N = 6$--$80$ demonstrates $\Delta E/\text{gap} < 5\%$ within the valid operating regime. PEA-ZNE error mitigation achieves +95\% mean gain ($R^2 = 0.998$) across all topologies. The framework extends to Ising Hamiltonian variants (longitudinal field: fidelity $\geq 0.98$) and demonstrates zero-shot cross-$N$ generalization (25/25 PASS for unseen system sizes). We identify the HVA $p = 2$ expressibility boundary as a physics limit, independently confirmed by Tripathi et al.\ (2026).
...
\textbf{Scaling ($N = 6$--$80$):} Accuracy \emph{improves} with system size: 2.7\% at $N = 10$ to 0.08\% at $N = 80$, explained by the trivial paramagnetic landscape at $h \gg h_c$. The valid regime follows $h_{\min} = 1.5 + 0.020 \cdot N^{1.31}$ ($R^2 = 1.0$).

\textbf{Zero-shot cross-$N$:} Training on $N = 40 + 80$ (14 points), the MPNN predicts for $N = 50$--$100$ with 25/25 PASS (mean 0.16\%), using \texttt{norm\_type="none"} to avoid BatchNorm artifacts on regular graphs.

\textbf{Error mitigation:} PEA-ZNE achieves +95\% mean gain ($R^2 = 0.998$) across 4 topologies (18/18 wins vs.\ gate-folding, $t = 46.3$, $p < 10^{-19}$), confirming it as the optimal strategy for this circuit class \citep{kim2023}.
...
Compared to literature: NN-VQE \citep{miao2024} validates MLP prediction at $N \leq 10$; Qracle \citep{zhang2025} validates GNN initialization reducing 64\% of iterations. Our contribution is the integration and systematic validation across $N = 6$--$80$, 5 topologies, and with PEA-ZNE mitigation --- extending the paradigm rather than inventing it.

\section*{5. Conclusions}

GNN-HVA demonstrates that the integration of GNN-based parameter prediction \citep{miao2024, zhang2025}, shallow HVA circuits \citep{wiersema2020, mele2026}, and PEA-ZNE mitigation \citep{kim2023} into a unified pipeline enables quantum phase classification on NISQ hardware with near-zero quantum optimization cost. The pipeline scales to $N = 80$ without architectural changes, PEA-ZNE provides +95\% noise reduction, and the framework extends to Ising variants. The approach is directly deployable on IBM Torino (133 qubits, heavy-hex native) for mapping spin-system phase diagrams. Code available at: \url{https://github.com/franraineri/qmbp_gnn_adapt-vqe}.
