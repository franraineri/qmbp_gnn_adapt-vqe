...
\begin{enumerate}
    \item \textbf{El pipeline GNN-HVA clasifica correctamente fases cuánticas} con $\Delta E/\text{gap} < 5\%$ en el régimen operativo válido, validado sobre 430+ ejecuciones, 5 topologías y tamaños de sistema de $N = 6$ a $N = 80$ (Tablas~\ref{tab:cross_topo}--\ref{tab:scaling}). La tasa de aprobación es 100\% dentro del régimen válido ($h \geq h_{\min}$) para configuraciones optimizadas.
...
\begin{itemize}
    ...
    \item \textbf{OE5} (Validación sistemática): Completado --- 430+ ejecuciones, 5 topologías, $N = 6$--$80$, 49 experimentos formales.
