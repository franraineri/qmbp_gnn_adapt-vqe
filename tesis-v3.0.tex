...
\textbf{Arquitectura:}
\begin{itemize}
    \item 3 capas GINConv \citep{xu2019} + \texttt{global\_mean\_pool} (agregación que promedia las representaciones de todos los nodos para producir un vector de tamaño fijo independiente de $N$) + MLP de salida.
...
En total, se acumulan 476+ ejecuciones de pipeline documentadas en bitácoras estructuradas, con 49 experimentos formales y una tasa de resultados útiles del 84\% (véase Tabla~\ref{tab:experiment_summary} en Capítulo~5).
...
El pipeline se evaluó sobre cuatro topologías a $N = 10$ con configuración unificada (hidden\_dim $= 128$, restarts $= 5$, patience $= 500$ (epochs sin mejora en validación antes de parada temprana), epochs $= 6000$, lr $= 10^{-3}$, dropout $= 0.1$). La Tabla~\ref{tab:cross_topo} resume los resultados de las 15 mejores variantes por topología.
...
\caption{Escalamiento del pipeline con el tamaño del sistema. La precisión mejora con $N$ (de 2.7\% a 0.019\%) debido a la simplificación del paisaje energético en la fase paramagnética profunda. La frontera del régimen válido sigue $h_{\min} = 1.5 + 0.020 \cdot N^{1.31}$ ($R^2 = 1.0$), validada de $N = 40$ a $N = 200$.}
...
El hallazgo clave es que \textbf{BatchNorm es perjudicial para generalización cross-N} en topologías con simetría nodal. La normalización por lotes (BatchNorm) es una técnica estándar en redes neuronales profundas que re-escala las activaciones internas de cada capa para que tengan media cero y varianza unitaria, estabilizando y acelerando el entrenamiento. Sin embargo, en cadena 1D todos los nodos son idénticos post-GINConv (la cadena es un grafo regular donde cada nodo interior tiene exactamente 2 vecinos con el mismo atributo $h$), causando que todas las representaciones nodales sean numéricamente iguales. Bajo esta condición, la varianza intra-grafo es exactamente cero, y BatchNorm intenta dividir por esta varianza nula ---produciendo activaciones corrompidas o indefinidas que degradan la predicción. Además, durante inferencia en grafos de tamaño $N$ distinto al de entrenamiento, las estadísticas acumuladas (\emph{running mean/var}) capturan artefactos del tamaño de grafo de entrenamiento, no variación física genuina. Este comportamiento es consistente con la observación de \citet{xu2019} de que GIN opera sobre grafos regulares mediante sumas globales ---la normalización destruye la señal cuando todos los nodos tienen representaciones idénticas. La solución empírica es desactivar la normalización (\texttt{norm\_type="none"}), que reduce el error de 18.5\% a 0.13\% (factor $142\times$ de mejora).
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{tesis-figures/fig_cross_n_zero_shot_bar.pdf}
\caption{Comparación GNN (sin BatchNorm) vs. interpolación clásica (scipy, que computa interpolación lineal 1D punto a punto entre los $h$-valores de entrenamiento) para generalización cross-N. Ambos métodos logran $\Delta E/\text{gap} \ll 5\%$ en el régimen de 2 parámetros ($p = 1$), pero la MPNN es la única opción viable para el régimen bond-resolved de 79 dimensiones donde la interpolación clásica no puede operar.}
\label{fig:cross_n_bar}
\end{figure}
...
Para topologías complejas o valores de $p$ donde se requieren parámetros por enlace (bond-resolved), el espacio paramétrico escala como $2p \cdot |E|$ dimensiones ($|E|$ = número de aristas del grafo). A $N = 40$, $p = 2$ sobre cadena 1D, esto produce 79 parámetros ---un régimen donde la interpolación clásica (scipy: interpolación lineal multidimensional a partir de puntos vecinos) es geométricamente impracticable por la maldición de la dimensionalidad.
...
La calidad de la Fase 2 (VQE) se caracteriza mediante la suavidad de los parámetros $\theta_{\text{smooth}}$ (Figura~\ref{fig:vqe_landscape}): valores $> 1.0$ indican ``chain breaks'' que degradan el entrenamiento posterior de la MPNN. El 29\% de las ejecuciones presenta chain breaks, pero el sistema de early-stopping los detecta en tiempo real.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{tesis-figures/fig_vqe_convergence_landscape.pdf}
\caption{Calidad de convergencia VQE. Izquierda: distribución de $\theta_{\text{smooth}}$ para 329 runs; el 29\% supera el umbral de chain break ($\theta > 1.0$). Derecha: correlación entre $\theta$-smoothness y $\Delta E/\text{gap}$ final --- los chain breaks (puntos rojos) generan errores significativamente mayores.}
\label{fig:vqe_landscape}
\end{figure}

\subsection{Resultados a $N = 6$ (Cadena 1D, HVA $p = 2$)}

El pipeline se validó sobre cadena de $N = 6$ espines con HVA $p = 2$, barrido de 27 puntos en $h \in [0, 2]$, y predictor MPNN (GINConv, hidden\_dim $= 64$, 3 capas, 6000 epochs). Se ejecutaron 40+ experimentos sobre 14 configuraciones. Los resultados (Tabla~\ref{tab:results_baseline}) muestran que el pipeline logra $\Delta E/\text{gap} < 5\%$ para todos los puntos de test, con precisión decreciente al acercarse a la frontera de expresividad ($h = 1.25$).
...
\begin{table}[h]
\centering
\caption{Pipeline GNN-HVA: resultados consolidados $N = 6$ y $N = 10$, cadena 1D, $p = 2$ (media $\pm$ std sobre 3 semillas).}
\label{tab:results_baseline}
\begin{tabular}{clccc}
\toprule
\textbf{$N$} & \textbf{$h_{\text{test}}$} & \textbf{$\Delta E/\text{gap}$} & \textbf{Checklist} & \textbf{Estado} \\
\midrule
6 & 1.25 & $3.77\% \pm 0.53\%$ & 4/4 & \checkmark \\
6 & 1.40 & $1.71\% \pm 0.14\%$ & 4/4 & \checkmark \\
6 & 1.50 & $1.19\% \pm 0.23\%$ & 4/4 & \checkmark \\
\midrule
10 & 1.40 & $4.79\% \pm 0.50\%$ & 4/4 & \checkmark \\
10 & 1.50 & $2.96\% \pm 0.33\%$ & 4/4 & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{tesis-figures/fig_zne_pea_forest_plot.pdf}
\caption{Forest plot de ganancia ZNE por topología con intervalos de confianza al 95\%. PEA (círculos) supera consistentemente a Gate-Folding (cuadrados) en todas las topologías, con intervalos no solapantes. La menor varianza de PEA confirma su robustez como estrategia primaria de mitigación.}
\label{fig:zne_forest_plot}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.75\textwidth]{tesis-figures/fig_zne_gain_heatmap.pdf}
\caption{Ganancia ZNE por topología y estrategia (PEA vs. Gate-Folding). PEA logra ganancias $>90\%$ en todas las topologías, mientras que Gate-Folding se limita a $+17$--$24\%$. El número entre paréntesis indica las evaluaciones disponibles por celda.}
\label{fig:zne_heatmap}
\end{figure}
...
Adicionalmente, la ablación del componente de grafo demuestra su esencialidad en modo predictivo (sin $E_{\text{noisy}}$ como entrada): la GNN (GINConv) logra 100\% de improvement rate, frente al 67\% del MLP y 0\% del modelo lineal (Figura~\ref{fig:gnn_ablation}). Este resultado confirma que la topología del Hamiltoniano ---capturada por el paso de mensajes--- es información necesaria para la corrección de errores cuando no se dispone de la energía ruidosa como feature.

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{tesis-figures/fig_gnn_qem_ablation_bar.pdf}
\caption{Ablación de GNN-QEM en modo predictivo (sin $E_{\text{noisy}}$): la arquitectura de grafos es esencial. GNN: 100\% improvement rate (+65\% reducción de error); MLP: 67\% (+20\%); Lineal: 0\% ($-$12\%, empeora). La estructura del grafo captura patrones de propagación de ruido inaccesibles a modelos no-grafos.}
\label{fig:gnn_ablation}
\end{figure}
...
\textbf{Tipo de experimento:} Análisis de compilación de circuitos (FakeTorino, $N = 10$, $p = 1$, heavy-hex, \texttt{optimization\_level=2}). Se evaluaron 4 estrategias de transpilación para reducir la profundidad del circuito HVA en hardware.

\textbf{Hallazgos:} La representación \texttt{PauliEvolutionGate} reduce la profundidad 2Q en 11\% (de 27 a 24) sin cambiar el conteo de compuertas (34 gates 2Q), porque el transpilador reconoce que las rotaciones ZZ conmutan y las programa en paralelo. Level 3 (KAK resynthesis) no ofrece beneficio y el plugin Rustiq es contraproducente (duplica las compuertas). Se adopta \texttt{PauliEvolutionGate} como estándar para despliegue. Los resultados cuantitativos completos se detallan en el Anexo A (auditoría de transpilación).
...
El diagnóstico automatizado sobre 174 ejecuciones clasificó 76 fallos por causa raíz. La Figura~\ref{fig:failure_treemap} muestra la distribución: el 69\% de los fallos es prevenible sin perder ningún run exitoso (CHAIN\_BREAK 45\% detectable en Fase 2 + MPNN\_OVERFIT 25\% detectable en Fase 3). La tasa de resultados útiles (confirmados + negativos válidos) es del 84\% (41/49 experimentos formales producen conocimiento accionable), comparado con una tasa típica del 50--70\% reportada en estudios de benchmarking VQE \citep{tilly2022}.
...
La MPNN demuestra alta eficiencia de datos: logra $\Delta E/\text{gap} < 5\%$ con tan solo 5 puntos de entrenamiento para semillas favorables, y la recomendación conservadora es $k = 7$--$9$ puntos para robustez cross-seed (reducción de 47--59\% respecto al grid estándar de 17 puntos). A $N = 10$, el $\Delta E/\text{gap}$ medio varía solo entre 2.94\% ($k = 17$, completo) y 3.37\% ($k = 5$, mínimo viable), confirmando que la función $h \mapsto \theta^*$ es suficientemente suave para aprenderse con pocas muestras (véase Anexo A para la tabla completa por $k$).
...
Un baseline importante es la interpolación lineal de $\theta^*(h)$ a partir de los puntos de entrenamiento. La interpolación (implementada con \texttt{scipy.interpolate.interp1d}) construye una función lineal a tramos entre los puntos de entrenamiento conocidos $(h_i, \theta^*_i)$ y evalúa en puntos intermedios sin entrenamiento de modelo:
...
\textbf{Tipo de análisis:} Derivado de simulación noiseless (StatevectorEstimator, $N = 4$--$10$, $p = 2$, todas las topologías). Se identifica la frontera de expresividad del ansatz HVA a partir de la fidelidad entre el estado preparado y el ground truth exacto en función de $h$.
...
\textbf{Tipo de análisis:} Post-procesamiento clásico sobre parámetros de la MPNN entrenada con datos noiseless (StatevectorEstimator). No requiere evaluaciones adicionales de circuito --- análisis puramente clásico del modelo entrenado. Costo QPU: cero (reutiliza datos existentes de Fase 2).
...
La convergencia del pico PCA hacia $h_c = 1.0$ se confirma cuando el grid de $h$ cubre la transición: a $N = 100$ con $h \in [1.0, 3.0]$, el pico PCA se localiza en $h = 1.033 \pm 0.047$ (3 semillas), a solo $\Delta h = 0.033$ del valor teórico \citep{dutta2015}. En contraste, a $N = 40$--$80$, el grid operativo ($h \geq h_{\min} \approx 4$--$8$) no cruza $h_c$, por lo que el pico aparece en el borde del grid (no detección). La Figura~\ref{fig:pca_detection} muestra esta convergencia y la amplitud de la derivada $|\partial\theta/\partial h|$ en función de $N$.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{tesis-figures/fig_pca_phase_detection.pdf}
\caption{Detección no supervisada de la transición de fase vía PCA de $\theta_{\text{opt}}(h)$. Izquierda: el pico PCA converge a $h_c = 1.0$ con $N$ (a $N = 100$: $\Delta h = 0.033$). Derecha: amplitud de $|\partial\theta/\partial h|_{\max}$ por tamaño de sistema. La detección requiere que el grid de $h$ cubra la región crítica --- condición satisfecha solo para $N \leq 10$ bajo la restricción de régimen válido.}
\label{fig:pca_detection}
\end{figure}

Este resultado confirma que la detección PCA es un método válido de costo cero cuántico, pero requiere datos VQE que abarquen la región crítica ---condición satisfecha solo para $N \leq 10$ bajo la restricción de régimen válido ($h_{\min}$).
...
\textbf{Tipo de análisis:} Evaluación clásica de la MPNN entrenada (50 pasadas forward con dropout activo). Los datos subyacentes provienen de simulación noiseless (StatevectorEstimator, $N = 6$, $p = 2$, cadena 1D, 3 semillas). El MC-Dropout opera exclusivamente sobre la red neuronal --- no requiere evaluaciones de circuito cuántico.
...
\begin{table}[h]
\centering
\caption{Resumen de la campaña experimental formal (49 experimentos con hipótesis formales). Las 476+ ejecuciones totales incluyen además los experimentos de escalamiento (V7), Heisenberg (V9), S-series (S1-S8), Tier-1 (T1a-T1c), y cross-topology/zero-shot.}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{tesis-figures/fig_scaling_timing_power_law.pdf}
\caption{Escalamiento del costo computacional de la Fase 2 (VQE) con el tamaño del sistema en escala log-log. El fit de ley de potencias muestra $T(N) \propto N^{\alpha}$, con $\alpha \approx 2.5$. A $N = 80$ el costo es de solo 109s gracias a la trivialidad del paisaje paramagnético profundo.}
\label{fig:timing_power_law}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{OE5} (Validación sistemática): Completado --- 476+ ejecuciones, 5 topologías, $N = 6$--$200$, 49 experimentos formales.
...
\caption{Estado de corroboración de los 23 hallazgos principales de la tesis, validados mediante tests estadísticos automatizados contra datos crudos. 21/23 findings están corroborados con evidencia fuerte; 2 (transferencia cross-topology y tasa de éxito experimental) se clasifican como cualificados por tratarse de resultados con caveats metodológicos documentados.}
