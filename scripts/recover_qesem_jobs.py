...
def recover_qesem_job(catalog, job_id: str, verbose: bool = False) -> dict:
    ...
    try:
        ...
        for idx, pub_result in enumerate(result):
            ...
            stds = np.atleast_1d(
                pub_result.data.stds if hasattr(pub_result.data, "stds") else np.zeros_like(evs)
            )
            ...
            if (
                transpiled_circs
                and isinstance(transpiled_circs, list)
                and len(transpiled_circs) > 0
            ):
                ...
                for line in qasm_str.split("\n"):
                    ...
                    if line and not line.startswith(
                        ("//", "OPENQASM", "include", "gate", "bit", "qubit", "}", "{")
                    ):
                        qasm_depth_lines += 1
