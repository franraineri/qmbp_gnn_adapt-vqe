...
# By kind
python -m scripts.digest --kind noiseless
python -m scripts.digest --kind noisy
python -m scripts.digest --kind experiment
```
...
python -m scripts.digest --kind noiseless --group-by n_restarts --topology ladder
...
python -m scripts.digest --kind noiseless --stats
...
# Save to file
python -m scripts.digest --kind noiseless -o results_summary.txt
...
python -m scripts.digest --kind experiment --verbose
```
...
```python
from scripts.digest import ResultScanner, NoiselessResult
