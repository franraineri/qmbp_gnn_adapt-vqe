...
Run with: python -m scripts.digest [options]
Or:       python scripts/digest/__main__.py [options]
"""
...
from scripts.digest.formatters import (
    format_compare_two,
...
from scripts.digest.models import NoiselessResult, NoisyResult
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
    python -m scripts.digest                         All results by kind
    python -m scripts.digest --kind noiseless        Only noiseless runs
    python -m scripts.digest --kind noisy            Only noisy/ZNE
    python -m scripts.digest --kind experiment       Only hypothesis tests
    python -m scripts.digest --topology ladder       Filter by topology
    python -m scripts.digest --n-qubits 10           Filter by system size
    python -m scripts.digest --folder variants_N10_ladder
    python -m scripts.digest --sort delta_e          Sort by ΔE/gap
    python -m scripts.digest --markdown -o digest.md
    python -m scripts.digest --json digest.json
    python -m scripts.digest --verbose
        """,
    )
