...
Usage:
    python scripts/maintenance/cleanup_repo.py              # Dry-run (show what would be removed)
    python scripts/maintenance/cleanup_repo.py --execute    # Actually delete
    python scripts/maintenance/cleanup_repo.py --verbose    # Show all scanned paths
"""
...
def main() -> None:
    ...
    report.files_found = [
        ...
        if not any(f.is_relative_to(d) for d in dirs_set)
    ]
    report.empty_dirs_found = [
        d for d in report.empty_dirs_found
