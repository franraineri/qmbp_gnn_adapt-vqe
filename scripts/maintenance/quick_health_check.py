...
def check_training_data(topo_filter: str | None = None) -> dict:
    ...
    for npz_file in sorted(npz_dir.glob("*.npz")):
        ...
        try:
            ...
            category, reason = classify_training_utility(n_pts, pass_dual, pass_5pct)
...
def print_report(zoo: dict, training: dict, gt: dict, index: dict, verbose: bool = False):
    ...
    # Model Zoo
    print(f"\n📦 MODEL ZOO")
    print(f"   Entries: {zoo['n_entries']} | Valid: {zoo['n_valid']} | "
          f"Missing: {zoo['n_missing']} | Corrupted: {zoo['n_corrupted']}")
    print(f"   Multi-N models: {zoo['n_multi_n']} | Unvalidated: {zoo['n_unvalidated']} | "
    ...
    # Training Data
    print(f"\n📊 TRAINING DATA")
    print(f"   Files: {training['total_files']} | Points: {training['total_points']} | "
          f"Good: {training['total_good']}")
    status = "✅" if training['n_not_useful'] == 0 else "⚠️"
    print(f"   {status} Useful: {training['n_useful']} | "
    ...
    # Result Index
    print(f"\n📋 RESULT INDEX")
    print(f"   Valid runs: {index.get('n_valid_runs', 0)} | Without tag: {index.get('n_without_tag', 0)}")
...
def main():
    ...
    if args.json:
        ...
        print_report(zoo, training, gt, index, verbose=args.verbose)
