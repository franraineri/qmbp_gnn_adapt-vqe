...
- MT vs ST head-to-head section (if comparison data exists)
...
Usage:
    ...
    # ── MT vs ST Analysis (after running model comparisons) ───────────────

    # Evaluate zoo + auto-update pass_rates + generate MT vs ST report:
    .venv/bin/python scripts/analysis/evaluate_zoo_models.py --update-zoo

    # Evaluate only heavy_hex models (targeted topology check):
    .venv/bin/python scripts/analysis/evaluate_zoo_models.py --topology heavy_hex

    # After evaluation, inspect MT vs ST from Python:
    #   from qmbp_simulation.analysis.evaluation_report import generate_mt_vs_st_table
    #   lines, s = generate_mt_vs_st_table(topology_filter="heavy_hex", n_min=16)
    #   print('\\n'.join(lines))

    # Generate standalone MT vs ST markdown (all topologies, N<=20):
    #   lines, s = generate_mt_vs_st_table(n_max=20, output_path="results/mt_vs_st.md")
"""
...
def format_markdown_report(all_results: list[dict], elapsed_s: float) -> str:
    ...
    # ── MT vs ST Head-to-Head (if comparison data exists) ─────────────────
    try:
        from qmbp_simulation.analysis.evaluation_report import generate_mt_vs_st_table

        mt_lines, mt_summary = generate_mt_vs_st_table(latest_only=True)
        if mt_summary.get("total", 0) > 0:
            lines.append("")
            lines.extend(mt_lines)
            lines.append("")
    except Exception:
        pass  # No comparison data available — skip silently
...
def main() -> int:
    ...
    # ── Pre-analysis: ensure NPZ e_exact matches GT cache ────────────────
    try:
        ...
        if coherence["n_points_fixed"] > 0:
            print(
                f"  [pre-check] Fixed {coherence['n_points_fixed']} stale e_exact points "
                f"in {coherence['n_files_with_issues']} NPZ files (GT cache is authoritative)"
            )
    except Exception:
        pass
