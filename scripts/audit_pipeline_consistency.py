...
if spsa_run.exists():
    ...
    print(f"  SPSA run (run_20260614_201051):")
...
for desc, nf, vals in test_cases:
    ...
    print(f"  {desc:20s} | {extrap_lin:10.4f} | {extrap_exp:12.4f} | {exp_deviation:10.4f} | {status}")
...
for run_dir in sorted(results_dir.glob("run_*")):
    ...
    print(f"  {run_dir.name}: e_zne={e_zne_run:.3f}, e_recon={e_reconstructed:.3f}, "
...
print(f"  {'h':>5} | {'Canonical (A)':>14} | {'Benchmark (B)':>14} | {'Difference':>11} | {'Impact':>30}")
...
if folded_1.depth() == qc.depth() and folded_1.size() == qc.size():
    ...
    warn(f"noise_factor=1: depth {qc.depth()}→{folded_1.depth()}, size {qc.size()}→{folded_1.size()}")
...
if high_r2_high_error:
    ...
    for r in high_r2_high_error:
        print(f"    {r['run']}: R²={r['r2']:.4f}, ΔE/gap={r['true_delta']*100:.2f}%, "
...
findings = [
    ("SPSA bypass of affine correction",
     "LOW", "Code path skips affine clip after SPSA. No impact on current runs "
     "(SPSA result was above e_ground), but could cause variational violations "
     "in future runs."),
    ("Exponential extrapolation",
     "NONE", "Falls back to linear correctly. No wild values produced."),
    ("Hardware summary data integrity",
     "INFO", "All stored data has correct dimensions and physical bounds."),
    ("Cross-validation formula",
     "NONE", "Formula matches TFIM Hamiltonian. Discrepancies within 2×gap."),
    ("Stale delta_e_gap from affine bug",
     "INFO", "3 runs have wrong delta_e_gap/verdict in JSON (never re-written). "
     "Any analysis must recompute from e_zne."),
    ("_compute_upper_bound inconsistency",
     "LOW", "Benchmark uses different formula than canonical (tighter bound). "
     "No practical impact for h≥3.25 (energy far below both bounds)."),
    ("fold_gates correctness",
     "NONE", "All edge cases handled correctly."),
    ("R² proxy reliability",
     "DESIGN", "R² measures precision, not accuracy. Consistent bias gives "
