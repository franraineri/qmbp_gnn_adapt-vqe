...
def save_raw_job_output(job, save_dir: str | Path, label: str = "") -> Path | None:
    ...
    try:
        ...
        for idx, pub_result in enumerate(job_result):
            ...
            stds_val = (
                float(stds) if stds is not None and np.isscalar(stds)
            ...
            for extra in ("evs_noise_factors", "stds_noise_factors",
                          "ensemble_stds", "metadata"):
...
def retrieve_job(service, job_id: str, verbose: bool = False) -> dict:
    ...
    for idx, pub_result in enumerate(job_result):
        ...
        evs_val = float(evs) if np.isscalar(evs) else evs.tolist()
        stds_val = float(stds) if stds is not None and np.isscalar(stds) else (
            stds.tolist() if stds is not None else None
        )
        ...
        for attr in ("evs_noise_factors", "stds_noise_factors",
                     "ensemble_stds", "metadata"):
...
def main():
    parser = argparse.ArgumentParser(
        description="Recover results from a completed IBM Runtime job"
    )
    parser.add_argument(
        "job_ids",
        nargs="+",
