...
from scripts.experiments_v8.techniques.hessian_restart import (
    hessian_guided_vqe,
...
class ExperimentB4(BaseExperiment):
    ...
    def run_single(self, seed: int) -> list[V8Metrics]:
        ...
        for h in sorted(h_values, reverse=True):  # Descending
            ...
            min(result_standard["energy"], result_hessian["energy"])
            best_theta = (
    ...
    def analyze(self, results: dict[int, list[V8Metrics]]) -> dict:
        ...
        # Aggregate restart and evaluation savings
        all_restarts_used = []
        all_eval_savings = []
        all_accuracy_diff = []
        all_true_minima = []
        ...
        for _seed, metrics in results.items():
            for m in metrics:
                ...
                all_restarts_used.append(md["hessian_restarts_used"])
                all_eval_savings.append(md["eval_savings_pct"])
                all_accuracy_diff.append(md["hessian_de_gap"] - md["standard_de_gap"])
                all_true_minima.append(md["is_true_minimum"])

        analysis["hessian_summary"] = {
            "mean_restarts_used": float(np.mean(all_restarts_used)),
            "mean_eval_savings_pct": float(np.mean(all_eval_savings)),
            "mean_accuracy_diff": float(np.mean(all_accuracy_diff)),
            "fraction_true_minima": float(np.mean(all_true_minima)),
            "hypothesis_confirmed": (
                float(np.mean(all_restarts_used)) <= 3.0
                and float(np.mean(all_accuracy_diff)) < 0.01
            ),
        }
    ...
    def report(self, analysis: dict) -> str:
        ...
        lines = [base, "", "Hessian-Guided Analysis:"]

        hs = analysis.get("hessian_summary", {})
        lines.extend(
            [
                f"  Mean restarts used:     {hs.get('mean_restarts_used', 0):.1f} (vs 5 standard)",
                f"  Mean eval savings:      {hs.get('mean_eval_savings_pct', 0):.1f}%",
                f"  Mean accuracy diff:     {hs.get('mean_accuracy_diff', 0):.4f} (should be ~0)",
                f"  Fraction true minima:   {hs.get('fraction_true_minima', 0) * 100:.0f}%",
                f"  Hypothesis confirmed:   {hs.get('hypothesis_confirmed', False)}",
            ]
        )
