...
from scripts.experiments_v8.techniques.sign_equivariant import (
...
class ExperimentC3(BaseExperiment):
    ...
    def run_single(self, seed: int) -> list[V8Metrics]:
        ...
        for h in sorted(h_train, reverse=True):  # Descending
            ...
            self.logger.info(f"    h={h:.2f}: E={best_energy:.4f}, θ={best_theta}")

        # Detect sign inconsistency
        ...
        for h_t in h_test:
            ...
            e_cold = cost_fn_mps(cold_theta, H)
            ...
            m = V8Metrics(
                ...
                technique_metadata={
                    ...
                    "warm_vs_cold_gain_pct": float(
                        (de_gap_cold - de_gap_canon) / max(de_gap_cold, 1e-10) * 100
                    ),
    ...
    def analyze(self, results: dict[int, list[V8Metrics]]) -> dict:
        ...
        # Aggregate canonicalization effect
        raw_errors = []
        canon_errors = []
        cold_errors = []
        ...
        for _seed, metrics in results.items():
            for m in metrics:
                ...
                raw_errors.append(md["de_gap_raw"])
                canon_errors.append(md["de_gap_canonicalized"])
                cold_errors.append(md["de_gap_cold"])

        analysis["canonicalization_effect"] = {
            "mean_de_gap_raw": float(np.mean(raw_errors)),
            "mean_de_gap_canonicalized": float(np.mean(canon_errors)),
            "mean_de_gap_cold": float(np.mean(cold_errors)),
            "pass_rate_raw": float(np.mean([e < 0.05 for e in raw_errors])),
            "pass_rate_canonicalized": float(np.mean([e < 0.05 for e in canon_errors])),
            "mean_improvement_pct": float(
                (np.mean(raw_errors) - np.mean(canon_errors))
                / max(np.mean(raw_errors), 1e-10)
                * 100
            ),
        }
    ...
    def report(self, analysis: dict) -> str:
        ...
        lines = [base, "", "Canonicalization Effect:"]

        effect = analysis.get("canonicalization_effect", {})
        lines.extend(
            [
                f"  Raw (no canon):     mean ΔE/gap = {effect.get('mean_de_gap_raw', 0):.4f}, "
                f"pass rate = {effect.get('pass_rate_raw', 0) * 100:.0f}%",
                f"  Canonicalized:      mean ΔE/gap = {effect.get('mean_de_gap_canonicalized', 0):.4f}, "
                f"pass rate = {effect.get('pass_rate_canonicalized', 0) * 100:.0f}%",
                f"  Cold-start:         mean ΔE/gap = {effect.get('mean_de_gap_cold', 0):.4f}",
                f"  Improvement:        {effect.get('mean_improvement_pct', 0):.1f}%",
            ]
        )
