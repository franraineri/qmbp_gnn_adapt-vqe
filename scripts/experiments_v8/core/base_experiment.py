"""Abstract base class for all V8 experiments.
...
# Project root — find by searching for pyproject.toml
def _find_project_root() -> Path:
    """Find project root by searching for pyproject.toml."""
    current = Path(__file__).resolve().parent
    for _ in range(10):  # Limit search depth to avoid infinite traversal
    ...
    raise RuntimeError(
        f"Could not find project root (pyproject.toml) starting from {Path(__file__).resolve()}"
    )
...
from scripts.experiments_v8.core.auto_registry import RunSummary, StructuredLogger
from scripts.experiments_v8.core.config import ExperimentConfig
from scripts.experiments_v8.core.metrics import V8Metrics, WarmColdComparison

logger = logging.getLogger(__name__)


class BaseExperiment(ABC):
    """Abstract base for all V8 experiments.
    ...
    def __init__(self, config: ExperimentConfig):
        self.config = config
        self.results_dir = (
            Path(__file__).resolve().parents[1] / "results" / f"exp_{config.experiment_id.lower()}"
        )
    ...
    def run_single(self, seed: int) -> list[V8Metrics]:
        ...
        -------
        list[V8Metrics]
            One V8Metrics per h-value or per test point.
        """
        ...

    def run(self) -> dict[int, list[V8Metrics]]:
        """Run experiment across all configured seeds with checkpointing.

        Returns
        -------
        dict[int, list[V8Metrics]]
            Mapping seed -> list of metrics.
        """
        all_results: dict[int, list[V8Metrics]] = {}
        ...
        for i, seed in enumerate(self.config.seeds):
            ...
            try:
                ...
                self.slog.stop_timer(
                    ...
                    seed=seed,
                    data={"n_points": len(metrics), "elapsed_s": round(elapsed, 2)},
                )
    ...
    def analyze(self, results: dict[int, list[V8Metrics]]) -> dict[str, Any]:
        """Compute summary statistics and optional baseline comparison.

        Parameters
        ----------
        results : dict[int, list[V8Metrics]]
            Output from run().

        Returns
        -------
        dict with keys: summary, per_seed, comparison (optional)
        """
    ...
    def report(self, analysis: dict[str, Any]) -> str:
        ...
        if "error" in summary:
            ...
            for seed, data in analysis.get("per_seed", {}).items():
                ...
                lines.append(
                    f"  Seed {seed}: {status} mean={data['mean_de_gap']:.4f}, "
    ...
    def save(self, results: dict[int, list[V8Metrics]], analysis: dict[str, Any]) -> Path:
        """Save full results + analysis + structured logs to JSON.

        Returns path to saved file.
        """
        # Build run summary for quick querying
        run_summary = RunSummary.from_analysis(
            experiment_id=self.config.experiment_id,
            run_id=self._run_id,
            analysis=analysis,
            results=results,
        ...
        # Also save structured log separately for analysis tools
    ...
    def get_exact_solution(self, h: float, N: int | None = None):
        """Get cached exact solution for given h-value.

        Returns dict with keys: lattice, hamiltonian, exact (GroundTruthResult).
        """
        from src.poc.v6 import make_lattice
    ...
    def evaluate_energy(self, params: np.ndarray, hamiltonian) -> float:
        ...
        Uses a cached estimator instance to avoid repeated instantiation.
        """
    ...
    def run_warm_cold_comparison(
        ...
        ----------
        h : float
            Field strength for this point.
        seed : int
            Random seed (used for cold-start initialization).
        warm_init : np.ndarray
            Warm-start initial parameters (from previous h or MPNN).
        ...
        return WarmColdComparison.compute(
            h_value=h,
            seed=seed,
            warm_init=warm_init,
    ...
    def _save_checkpoint(self, seed: int, metrics: list[V8Metrics]) -> None:
    ...
    def _load_checkpoint(self, seed: int) -> list[V8Metrics] | None:
        ...
        if cp_path.exists():
            try:
                with open(cp_path) as f:
                    data = json.load(f)
                return [V8Metrics(**d) for d in data]
