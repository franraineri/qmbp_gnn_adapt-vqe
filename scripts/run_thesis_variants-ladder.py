...
# ═══════════════════════════════════════════════════════════════════════════
# Runner (same infrastructure as N=6 script)
# ═══════════════════════════════════════════════════════════════════════════
...
def print_variant_table(variants: list[PipelineVariant]) -> None:
    ...
    print(f"\n{'#':<4} {'ID':<20} {'Category':<10} {'Description'}")
    print("-" * 85)
    for i, v in enumerate(variants):
        print(f"{i:<4} {v.id:<20} {v.category:<10} {v.description}")
    print()


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pipeline variants for N=10 LADDER topology",
    ...
    if args.list:
        print("=" * 85)
        print(f"  PIPELINE VARIANTS — N={n_qubits}, topology={TOPOLOGY}")
        print("=" * 85)
        print(f"\n  Noiseless: {len(noiseless)} variants")
        print(f"  Noisy:     {len(noisy)} variants")
        print(f"  Extended:  {len(extended)} variants")
    ...
    if args.variant is not None:
        if args.variant >= len(all_variants):
            print(f"ERROR: Index {args.variant} out of range (0-{len(all_variants) - 1})")
            sys.exit(1)
    ...
    # Full run mode
    print("=" * 85)
    print(f"  EXHAUSTIVE VARIANT RUNNER — N={n_qubits}, topology={TOPOLOGY}")
    print("=" * 85)
    ...
    if not args.dry_run:
        print("  ⚠️  Estimated time: 60-90 minutes (N=10 ladder is slower than chain)")
        print()
