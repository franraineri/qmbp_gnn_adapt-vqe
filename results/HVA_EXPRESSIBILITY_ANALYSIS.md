...
**Empirical fits (MPS deterministic backend, χ=64, N=20-250):**
```
p=1: h_frontier = 2.36 + 0.0073·N   (R²=0.91, linear growth ~0.007/qubit)
p=2: h_frontier = 1.57 + 0.0050·N   (R²=0.95, linear growth ~0.005/qubit)
p=3: h_frontier ≈ 1.6 ± 0.1         (quasi-constant, independent of N)
p=4: h_frontier ≈ 1.4 ± 0.1         (quasi-constant, independent of N)
```

**Key finding**: For p≥3, h_frontier is independent of N. This is consistent with
the area law: in 1D, entanglement entropy at fixed h/h_c is independent of N.
If the HVA has sufficient layers (p≥3) to capture that entanglement, the frontier
does not move when scaling N.
...
### F3. For p≥3, h_frontier is independent of N (area law)

The frontier is quasi-constant at ~1.6 (p=3) and ~1.4 (p=4) across N=20-120.
For p=1-2, growth is linear with small slopes (0.007 and 0.005 per qubit respectively).
This is consistent with the area law: in 1D gapped systems, entanglement at fixed h/h_c
does not depend on system size.

Evidence: H_EXPR_MATRIX (MPS deterministic, χ=64, N=20-250, 3 seeds × 5-7 restarts).
